...
const OpenPositionsModal = ({
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className={`${theme === 'dark' ? 'bg-[#121212] border border-gray-800' : 'bg-white'} rounded-xl shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto`}>
                <div className={`flex justify-between items-center p-6 border-b ${theme === 'dark' ? 'border-gray-800' : ''}`}>
                    <h2 className={`text-2xl font-bold ${theme === 'dark' ? 'text-white' : 'text-gray-800'}`}>Open Positions</h2>
                    <button
                        onClick={onClose}
                ...
                <div className="p-6">
                    {openJobs.length === 0 ? (
                        <div className="text-center py-8">
                            <p className={`${theme === 'dark' ? 'text-gray-400' : 'text-gray-500'} text-lg`}>No open positions found</p>
                        </div>
                    ) : (
                        <div className="grid gap-6">
                            {openJobs.map((job, index) => (
                                <div key={job._id || job.id || index} className={`border ${theme === 'dark' ? 'border-gray-800 bg-[#1c1c1c]' : 'bg-white'} rounded-lg p-6 hover:shadow-md transition-all`}>
                                    <div className="flex justify-between items-start mb-4">
                                        <div className="flex-1">
                                            <h3 className={`text-xl font-semibold ${theme === 'dark' ? 'text-white' : 'text-gray-800'} mb-2`}>{job.title || 'No Title'}</h3>
                ...
                <div className={`flex justify-end p-6 border-t ${theme === 'dark' ? 'border-gray-800' : ''}`}>
                    <button
                        onClick={onClose}
                        className={`px-4 py-2 text-white rounded-xl ${theme === 'dark' ? 'bg-slate-700 hover:bg-slate-600' : 'bg-slate-600 hover:bg-slate-700'} transition-colors`}
                    >
                        Close
                    </button>
                </div>
