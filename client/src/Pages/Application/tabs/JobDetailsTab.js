...
const JobDetailsTab = ({ job }) => {
  ...
  return (
    <div className="max-w-4xl w-full px-4 mx-auto flex flex-col items-center ">
      {/* Header Section */}
      <div className={`w-full mb-8 border-b pb-6 ${theme === 'dark' ? 'border-gray-700' : 'border-gray-200'}`}>
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
          ...
          <span className={`px-3 py-1 rounded-full text-sm font-medium ${getStatusStyle(displayStatus)}`}>
            {displayStatus || 'N/A'}
          </span>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 gap-y-6 w-full">
        {/* Main Details Section */}
        <div className="md:col-span-2 space-y-6">
          {/* Job Overview */}
          <section className={`rounded-xl border p-4 sm:p-6 space-y-4 transition-colors duration-300 ${theme === 'dark' ? 'bg-white/10 border-gray-700' : 'bg-gray-100 border-gray-200 shadow-md'
            }`}>
          ...
          {/* Location Details */}
          <section className={`rounded-xl border p-4 sm:p-6 transition-colors duration-300 ${theme === 'dark' ? 'bg-white/10 border-gray-700' : 'bg-gray-100 shadow-md border-gray-200'
            }`}>
            <h2 className={`text-lg font-semibold flex items-center mb-4 ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>
              <svg className="w-5 h-5 mr-2 text-[#9333ea]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
