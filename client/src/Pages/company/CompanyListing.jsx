...
const CompanyListing = () => {
  ...
  const fetchCompanies = async () => {
    try {
      const response = await fetch(
        ...
        }/companies/get?page=${currentPage}&limit=${itemsPerPage}&search=${encodeURIComponent(
          searchTerm
        )}`
      );
      if (response.ok) {
        ...
        if (searchTerm && data.Companies.length > 0) {
          toast.success(
            `Found ${data.totalCount} companies matching "${searchTerm}"`
          );
  ...
  const handleDeleteCompany = async () => {
    ...
    try {
      ...
      if (response.ok) {
        toast.success(
          `Company "${deleteConfirmDialog.company.CompanyUserName}" deleted successfully`
        );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error(
        `Error ${isEditing ? "updating" : "adding"} company:`,
        error
      );
    }
  };
  ...
  return (
    <div
      className="px-8 py-4 w-full min-h-screen"
    ...
    >
      ...
      <div className="max-w-screen-2xl">
        {/* Header Section */}
        <div className="mb-6 h-auto sm:h-[15vh] flex flex-col sm:flex-row items-center rounded-xl p-4 bg-gray-200 shadow-md dark:bg-transparent  gap-4 sm:gap-0 justify-between border dark:border-gray-700">
          <div className="flex items-center w-full sm:w-auto gap-4">
            <ChartNoAxesCombined className="h-6 w-6 text-[#9333ea]" />
            <h1 className="text-xl sm:text-3xl font-bold dark:text-white text-black">
              Company Directory
            </h1>
          </div>
          <div className="flex flex-col sm:flex-row items-center gap-4 w-full sm:w-auto">
            <div className="relative w-full sm:flex-grow">
              ...
              <input
                ...
                className={`w-full sm:w-[30vw] pl-10 pr-4 py-2 rounded-xl border border-gray-200 focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all duration-200 ${theme === 'dark' ? 'bg-gray-700 text-white border-gray-600 hover:bg-gray-600' : 'bg-white shadow-lg web-text-black hover:bg-white'}`}
            ...
            <button
              ...
              className="flex items-center justify-center w-full sm:w-auto px-4 py-2 bg-[#9333ea] text-white rounded-xl hover:bg-purple-800 hover:text-white border border-white transition-colors duration-200 whitespace-nowrap shadow-sm"
            >
              <Plus size={20} />
        ...
        {companies.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
            {companies.map((company) => (
              <div
                key={company.id}
                className="bg-white dark:bg-white/5 rounded-xl shadow-sm hover:shadow-md transition-shadow duration-200 p-6 border border-gray-100 flex flex-col dark:border-white/10"
              >
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-3">
                    <div className="p-2 bg-gray-200 dark:bg-[#313131] rounded-xl">
                      <Building2 className="text-[#9333ea]" size={24} />
                    </div>
