...
export const ShortlistedDetails = () => {
    ...
    useEffect( () => {
        ...
        console.log( job_id );
        try {
            fetch( `${ process.env.BASE_URL }/users/user/${ candidate_id }` )
                .then( ( res ) => res.json() )
                .then( ( data ) => {
                    setCandidate( data )
                } )

            fetch( `${ process.env.BASE_URL }/jobs/current-job/${ job_id }` )
                .then( ( res ) => res.json() )
                .then( ( data ) => {
                    setJob( data )
                } )
    ...
    useEffect( () => {
        try {
            fetch( `${ process.env.BASE_URL }/application/all-application/` )
                .then( ( res ) => res.json() )
                .then( ( data ) => {
