    ...
    const sendConfirmationEmail = async (formData) => {
        ...
        try {
            ...
            const templateParams = {
                ...
                application_status: formData.applicationStatusId || "Submitted",
            };
    ...
    if (!loginData && !isRecruiterManager) {
        return (
            <div className="flex flex-col items-center justify-center p-6 bg-purple-50 dark:bg-slate-800/50 rounded-lg text-center border dark:border-slate-700">
                ...
                <h3 className="text-lg font-medium text-gray-800 dark:text-white mb-2">Login Required</h3>
                <p className="text-gray-600 dark:text-slate-400 mb-4">Please log in to apply for this position</p>
                <button
    ...
    if (successMessage) {
        return (
            <div className="flex flex-col items-center justify-center p-6 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg text-center">
                ...
                <h3 className="text-lg font-medium text-gray-800 dark:text-white mb-2">Application Submitted!</h3>
                <p className="text-gray-600 dark:text-slate-300 mb-2">{successMessage}</p>
                {emailStatus && (
                    <p className="text-sm text-purple-600 dark:text-purple-400">{emailStatus}</p>
                )}
                <p className="text-gray-500 dark:text-slate-500 text-sm mt-2">Redirecting to your applications...</p>
            </div>
        );
    }
        ...
        <>
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                ...
                <div className="space-y-2">
                    ...
                    <p className="text-xs text-gray-500 dark:text-slate-500">PDF, DOCX, or RTF (Max 5MB)</p>
                </div>
                ...
                {emailStatus && (
                    <div className="p-3 bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-800 rounded-md">
                        <p className="text-purple-700 dark:text-purple-400 text-sm">{emailStatus}</p>
                    </div>
                )}
