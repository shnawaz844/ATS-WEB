        ...
        <div className="bg-white rounded-t-xl shadow-sm">
          <div className="flex border-b">
            ...
            {/* <button
              ...
              className={`px-6 py-4 font-medium text-sm focus:outline-none ${activeTab === 'feedback'
                ? 'border-b-2 border-indigo-500 text-indigo-600'
                : 'text-gray-500 hover:text-gray-700 hover:border-gray-300 hover:border-b-2'
                }`}
            >
                  ...
                  {assignedInterviews?.interviews?.length > 0 ? (
                    assignedInterviews.interviews.map((feedback) => (
                      <tr key={feedback._id} className="hover:bg-gray-50">
                          ...
                          <div className="flex items-start">
                            ...
                            <div className="ml-4">
                              <div className="text-sm font-medium text-gray-900">
                                {feedback?.applicationID?.candidateID?.userName || "N/A"}
                              </div>
                              <div className="text-sm text-gray-500">{feedback.applicationID?.jobID?.title || "N/A"}</div>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm text-gray-900">{feedback?.interviewerID?.userName || "N/A"}</div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
              ...
              {filteredFeedbacks.length === 0 ? (
                <div className="text-center py-10">
                  <div className="text-gray-400 mb-2">
                    ...
                    <p>No feedback records found matching your criteria.</p>
                  </div>
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {filteredFeedbacks.map((feedback) => (
                    <div key={feedback._id} className="border border-gray-200 rounded-lg overflow-hidden hover:shadow-md transition">
                      <div className="border-b border-gray-200 bg-gray-50 px-4 py-3 flex justify-between items-center">
                        <div>
                          <h3 className="text-sm font-medium text-gray-700">{feedback?.applicationID?.candidateID?.userName || "Unknown Candidate"}</h3>
                          <p className="text-xs text-gray-500">{feedbackForm.feedbackTitle}</p>
                        </div>
                        <div className="text-xs text-gray-500">{feedback?.date}</div>
                      </div>
                      <div className="p-4">
                        <div className="flex justify-between items-start mb-2">
                          <h4 className="text-md font-semibold">{feedbackForm.feedbackTitle}</h4>
                          <div className="text-sm">{getRatingStars(feedback?.rating)}</div>
                        </div>
                        <p className="text-sm text-gray-700 mb-4 line-clamp-3">{feedback.feedback}</p>
